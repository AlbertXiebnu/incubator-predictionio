...
class HDFSModels(clientMap: Map[String,AnyRef], config: StorageClientConfig, prefix: String)
  extends Models with Logging {

  private final val fs = clientMap("HDFSClient").asInstanceOf[org.apache.hadoop.fs.FileSystem]
  private final val rootPath = clientMap("RootPath").asInstanceOf[String]
  private final val path_prefix = rootPath + "/models"

  def insert(i: Model): Unit = {
    try {
      val fsdos = fs.create(new Path(s"${path_prefix}/$prefix${i.id}"))
  ...
  def get(id: String): Option[Model] = {
    try {
      val p = new Path(s"${path_prefix}/$prefix$id")
      Some(Model(
        id = id,
