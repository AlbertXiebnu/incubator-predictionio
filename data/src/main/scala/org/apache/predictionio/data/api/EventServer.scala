...
import spray.http._
import spray.http.{FormData, MediaTypes, MultipartFormData, StatusCodes,BodyPart}
              ...
              respondWithMediaType(MediaTypes.`application/json`) {
                complete {
                  ...
                  val data = eventClient.futureGet(eventId, appId, channelId).map { eventOpt =>
                    eventOpt match {
                      case Some(event) => (StatusCodes.OK, event)
                      case None => (StatusCodes.NotFound, Map("message" -> "Not Found"))
                    }
//                    eventOpt.map( event =>
