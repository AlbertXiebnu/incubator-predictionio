...
import org.apache.predictionio.data.api.{EventServer, EventServerConfig, FileUploadServer,FileUploadConfig}
...
object Management extends EitherLogging {
  ...
  def fileuploadserver(fa: FileUploadArgs): ActorSystem = {
    info(s"Creating FileUpload Server at ${fa.ip}:${fa.port}")
    FileUploadServer.createFileUploadServer(FileUploadConfig(
      ip = fa.ip,
      port = fa.port,
      maxFileSize = fa.maxFileSize
    ))
  }
