...
case class ConsoleArgs(
  ...
  fileUploadServer: FileUploadArgs = FileUploadArgs(),
...
object Console extends Logging {
  def main(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[ConsoleArgs]("pio") {
      ...
      note("")
      cmd("fileserver").
        text("Launch an FileUpload Server at specific IP and port.").
        action{ (_, c) =>
          c.copy(commands = c.commands :+ "fileserver")
        } children (
          opt[String]("ip") action { (x,c) =>
            c.copy(fileUploadServer = c.fileUploadServer.copy(ip = x))
          } text("IP to bind to. Default: localhost"),
          opt[Int]("port") action { (x,c) =>
            c.copy(fileUploadServer = c.fileUploadServer.copy(port = x))
          } text("Port to bind to. Default: 7072"),
          opt[Int]("max-file-size") action{ (x,c) =>
            c.copy(fileUploadServer = c.fileUploadServer.copy(maxFileSize = x))
          } text("max file size in upload. Default: 1024M")
      )
      note("")
      cmd("adminserver").
    ...
    parser.parse(consoleArgs, ConsoleArgs()) map { pca =>
      ...
      val rv: Int = ca.commands match {
        ...
        case Seq("fileserver") =>
          Pio.fileuploadserver(ca.fileUploadServer)
  ...
  val helpText = Map(
    ...
    "fileserver" -> txt.fileserver().toString,
